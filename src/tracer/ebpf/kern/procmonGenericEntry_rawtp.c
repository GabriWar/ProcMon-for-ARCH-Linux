...
PopulateArguments(enum ProcmonArgTag type, unsigned long arg,
                  struct SyscallEvent *event, unsigned int *offset_ptr) {
  unsigned int len = 0;
  unsigned int str = 0;
  ...
  bpf_probe(event->buffer + *offset_ptr, len, (void *)src, str) *offset_ptr +=
      len;
...
set_eventArgs(unsigned long *a, const struct pt_regs *regs) {
  int ret = 0;
...
// ------------------------------------------------------------------------------------------
__attribute__((always_inline)) static inline uint64_t GetRunningState() {
  uint64_t *state = NULL;
...
// ------------------------------------------------------------------------------------------
__attribute__((always_inline)) static inline int IsProcmon() {
  char comm[16] = {0};
  bpf_get_current_comm(comm, sizeof(comm));

  if (comm[0] == 'p' && comm[1] == 'r' && comm[2] == 'o' && comm[3] == 'c' &&
      comm[4] == 'm' && comm[5] == 'o' && comm[6] == 'n') {
    return 1;
  }
...
// Checks if the specified pid matches the pid filter
// ------------------------------------------------------------------------------------------
__attribute__((always_inline)) static inline int MatchPidFilter(int pid) {
  for (int i = 0; i < MAX_PIDS; i++) {
    ...
    if (pidMapItem) {
      ...
      if (foundPid == -1) {
        ...
        break; // we can bail early since -1 indicates we've reached the end of
               // the pid items in the map
      }
    } else {
      break;
    }
...
// ------------------------------------------------------------------------------------------
__attribute__((always_inline)) static inline int CheckFilters(int pid) {
  //
...
__attribute__((flatten)) int
genericRawEnter(struct bpf_our_raw_tracepoint_args *ctx) {
  ...
  sysEntry->userStackCount =
      bpf_get_stack(ctx, &sysEntry->userStack,
                    MAX_STACK_FRAMES * sizeof(uint64_t), BPF_F_USER_STACK) /
      sizeof(uint64_t);
  ...
  for (int i = 0; i < 6; i++) {
    if (PopulateArguments(schema->types[i], a[i], sysEntry, &offset) ||
        i >= schema->usedArgCount) {
      break;
    }
  }
  ...
  //
  if (bpf_map_update_elem(&syscallsMap, &pidTid, sysEntry, BPF_ANY) !=
      UPDATE_OKAY) {
